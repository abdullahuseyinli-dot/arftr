...
from hac.crossing_event_verifier_numerical_v2 import (  # noqa: E402
    VARIANTS,
...
PROTOCOL = ROOT / "experiments/okutama_crossing_event_verifier_numerical_v2_protocol.json"
DEFAULT_RUN = ROOT / ".runs/research_20260919/crossing_event_verifier_numerical_v2"
...
def prepare(run: Path):
    ...
    paths = [PROTOCOL, ANCHOR, REVIEW / "NEXT_TRIAL_PROTOCOL.md", REVIEW / "NEXT_TRIAL_PROTOCOL.json",
             ...
             ROOT / "src/hac/crossing_event_verifier_numerical_v2.py",
             ROOT / "tests/test_crossing_event_verifier_numerical_v2.py",
             ROOT / "experiments/run_okutama_crossing_event_verifier.py",
             ROOT / ".runs/research_20260919/crossing_event_verifier_v1/numerical_abort.json",
             ROOT / ".runs/research_20260919/crossing_event_verifier_v1/NUMERICAL_AMENDMENT.md",
...
def summarize(run: Path):
    ...
    for check in audit["checks"]:
        if check["variant"] == VARIANTS[2]:
            require(check["solution"]["gradient_linf"] <= 1e-7, "original objective gradient did not meet unchanged tolerance")
    ...
    summary["fixed_mechanism_contrasts"] = {
        ...
        for reader in READERS}
    summary["numerical_amendment"] = protocol["numerical_amendment"]
    ...
    lines += ["", f"Retained ARFTR: {100*baseline['macro_f1']:.6f}% macro-F1, {baseline['errors']} errors; unchanged.",
              "Numerical history: original attempt aborted after8 fits at the iteration limit; no metrics released. This complete rerun changes only L-BFGS history10→108, preserving the objective, tolerance and iteration cap. The invalid attempt remains archived.",
    ...
    abort_artifact = ".runs/research_20260919/crossing_event_verifier_v1/numerical_abort.json"
    graph["nodes"].append({"id": "experiment:crossing_event_verifier_v1/numerical_abort", "kind": "invalid_run",
                           "label": "8 fits then numerical abort; no scientific outcome", "artifact": abort_artifact, "status": "invalid_not_scientific_negative"})
    ledger["records"].append({"experiment_id": "crossing_event_verifier_v1::numerical_abort", "family": "crossing_event_verifier_v1",
                              "artifact": abort_artifact, "outcome": "invalid_numerical_abort", "completed_gate_fits": 8, "outer_metrics_released": False})
    for key, result in results.items():
        nid = "experiment:crossing_event_verifier_numerical_v2/" + key
        ...
        ledger["records"].append({"experiment_id": "crossing_event_verifier_numerical_v2::" + key,
                                  "family": "crossing_event_verifier_numerical_v2", "artifact": artifact,
    ...
    primary_node = "experiment:crossing_event_verifier_numerical_v2/" + primary_key
    ...
    ledger["supplement"] = {"source": artifact, "records_added": 10, "all_fits": 45, "parent_partial_fits": 8, "primary_outcome": summary["status"]}
