...
from hac.center_episode_training_v2 import fit_episode

ROOT = Path(__file__).resolve().parents[1]
RUN = ROOT / ".runs/research_20260908/center_episode_v2"
MEMORY_RUN = ROOT / ".runs/research_20260908/evidence_memory"
PROTOCOL = ROOT / "experiments/okutama_center_episode_protocol_v2.json"
...
def validate_unchanged_learning_contract(protocol, original_protocol):
    keys = (
        "primary_rows",
        "class_order",
        "allowed_input_cache",
        "allowed_nested_base_cache",
        "cache_reuse_rule",
        "new_pixels_or_duration_or_labels",
        "neighbor_seconds",
        "center_index",
        "offline_future_context",
        "raw_frame_support",
        "arms",
        "primary_contrast",
        "matched_capacity",
    ...
    if any(protocol[key] != original_protocol[key] for key in keys):
        raise RuntimeError("Numerical-only v2 must preserve the entire original learning contract")
    for key, value in original_protocol["attention"].items():
        if protocol["attention"].get(key) != value:
            raise RuntimeError(
                "V2 changed a declared mechanism beyond its numerical implementation"
            )
    return canonical_hash({key: protocol[key] for key in keys})


def validate_numerical_evidence(protocol):
    version = protocol["numerical_version"]
    old_lock_path = ROOT / version["prior_v1_execution_lock"]
    if file_sha256(old_lock_path) != version["prior_v1_execution_lock_sha256"]:
        raise RuntimeError("Interrupted v1 execution lock changed")
    old_run = old_lock_path.parent
    failed = old_run / "models/expected_episode/fold-1/config-2/inner-1/request.json"
    if len(list((old_run / "models").rglob("receipt.json"))) != 97 or {
        path.name for path in failed.parent.iterdir()
    } != {"request.json"}:
        raise RuntimeError("Interrupted v1 completed/failed fit census changed")
    for path, expected in audit.read_json(old_lock_path)["source_sha256"].items():
        if file_sha256(Path(path)) != expected:
            raise RuntimeError(f"Interrupted v1 locked source/input changed: {path}")
    failure_path = ROOT / version["forensic_receipt"]
    failure = audit.read_json(failure_path)
    actual_path = ROOT / version["actual_failure_validation"]
    actual = audit.read_json(actual_path)
    if (
        actual["status"] != "FROZEN_ACTUAL_FAILURE_NUMERICAL_V2_VALIDATION_PASSED"
        or not actual["forensic_only"]
        or not actual["original_model_and_batch_replay_bit_exact"]
        or not actual["parameter_states_unchanged"]
        or actual["new_optimizer_steps"] != 0
        or actual["new_classifier_fits"] != 0
        or actual["classification_labels_used"] != 0
        or actual["classification_scores_calculated"]
        or actual["unchanged_tolerance"] != protocol["coefficient_bound_tolerance"]
        or max(
            actual["stable_maximum_attention_minus_survival"],
            actual["stable_maximum_final_neighbor_coefficient_minus_survival"],
        )
        > protocol["coefficient_bound_tolerance"]
        or actual["original_maximum_attention_minus_survival"]
        <= protocol["coefficient_bound_tolerance"]
        or actual["failure_receipt_sha256"] != file_sha256(failure_path)
        or actual["forensic_artifact_sha256"] != failure["artifact_sha256"]
        or not failure["diagnostic_only"]
        or failure["candidate_result"]
    ):
        raise RuntimeError("Actual failed-case numerical correction gate did not pass")
    hashes = {
        str(old_lock_path): file_sha256(old_lock_path),
        str(failed): file_sha256(failed),
        str(failure_path): file_sha256(failure_path),
        str(actual_path): file_sha256(actual_path),
    }
    for name, expected in actual["forensic_artifact_sha256"].items():
        path = failure_path.parent / name
        if file_sha256(path) != expected:
            raise RuntimeError("Frozen actual failure batch/state changed")
        hashes[str(path)] = expected
    replay_lock = audit.read_json(failure_path.parent / "replay_lock.json")
    if replay_lock["original_failed_request_sha256"] != file_sha256(failed):
        raise RuntimeError("Forensic replay request differs from interrupted v1")
    for name, expected in actual["source_sha256"].items():
        path = ROOT / name
        if file_sha256(path) != expected:
            raise RuntimeError("Actual-case verification source changed")
        hashes[str(path)] = expected
    replay_source = ROOT / "experiments/replay_okutama_episode_bound_failure.py"
    if file_sha256(replay_source) != replay_lock["instrumented_replay_source_sha256"]:
        raise RuntimeError("Forensic replay source changed")
    hashes[str(replay_source)] = file_sha256(replay_source)
    return hashes
...
def validate_fit_evidence(
    ...
    expected = {
        ...
        "training_code_sha256": file_sha256(ROOT / "src/hac/center_episode_training_v2.py"),
        "architecture_code_sha256": file_sha256(ROOT / "src/hac/stable_center_episode_memory.py"),
        "v1_neural_definition_sha256": file_sha256(ROOT / "src/hac/center_episode_memory.py"),
...
def preflight(run, protocol):
    ...
    learning_contract_hash = validate_unchanged_learning_contract(
        protocol, audit.read_json(ROOT / "experiments/okutama_center_episode_protocol.json")
    )
    numerical_evidence_hashes = validate_numerical_evidence(protocol)
    if (
    ...
    source_files.update(numerical_evidence_hashes)
    ...
    if (
        pilot["model_source_sha256"]
        != file_sha256(ROOT / "src/hac/stable_center_episode_memory.py")
        or pilot["inherited_v1_model_source_sha256"]
        != file_sha256(ROOT / "src/hac/center_episode_memory.py")
        or pilot["source_v1_protocol_sha256"]
        != file_sha256(ROOT / "experiments/okutama_center_episode_protocol.json")
        or pilot["pilot_source_sha256"]
        != file_sha256(ROOT / "experiments/pilot_okutama_stable_center_episode.py")
        or pilot["tests_sha256"] != file_sha256(ROOT / "tests/test_stable_center_episode_memory.py")
        or pilot["numerical_version"] != "direct-path-float64-v2"
    ):
        raise RuntimeError("Stable episode pilot does not match new/inherited sources")
    if (
        not pilot["synthetic_only"]
        or pilot["optimizer_steps"] != 0
        or pilot["dataset_rows_read"] != 0
        or pilot["classifier_fits"] != 0
    ):
    ...
    for arm in protocol["arms"]:
        ...
        if (
            ...
            or not result["finite_gradients"]
            or not result["state_unchanged"]
    ...
    files = [
        ...
        ROOT / "experiments/okutama_center_episode_protocol.json",
        ROOT / "src/hac/center_episode_training_v2.py",
        ROOT / "src/hac/center_episode_memory.py",
        ROOT / "src/hac/stable_center_episode_memory.py",
        ROOT / "tests/test_stable_center_episode_memory.py",
        ...
        ROOT / "tests/test_center_episode_training_v2.py",
        ROOT / "tests/test_center_episode_runner_v2.py",
    ...
    receipt = {
        "protocol": protocol,
        "unchanged_learning_contract_sha256": learning_contract_hash,
    ...
    for fold in protocol["outer_folds"]:
        ...
        for seed in protocol["outer_seeds"]:
            ...
            expected = {
                ...
                "v1_neural_definition_sha256": file_sha256(
                    ROOT / "src/hac/center_episode_memory.py"
                ),
                "boundary_loss": "unweighted_masked_BCE_with_logits",
